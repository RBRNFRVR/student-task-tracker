...
function taskCard(t) {
  ...
  const subjectHtml = t.subjectName
    ? `<span class="subject-badge" style="background:${t.subjectColor}22;color:${t.subjectColor}"><span class="subject-dot" style="background:${t.subjectColor}"></span>${escape(t.subjectName)}</span>`
    : '';
  const estHtml = t.estimatedMinutes
    ? `<span class="task-estimated">⏱ ${formatMinutes(t.estimatedMinutes)}</span>`
    : '';
  const checkClass = t.status === 'COMPLETED' ? 'checked' : t.status === 'IN_PROGRESS' ? 'in-progress' : '';
...
async function cycleStatus(id, nextStatus) {
  ...
  if (!task) return;
  const updated = { ...task, status: nextStatus, subjectId: task.subjectId || null };
  const res = await fetch(`/api/tasks/${id}`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(toTaskRequest(updated))
  });
  if (res.ok) {
...
async function handleTaskSubmit(e) {
  ...
  const subjectIdVal = document.getElementById('task-subject').value;
...
function confirmDelete(type, id) {
  ...
  btn.onclick = () => performDelete(type, id);
...
function formatDue(isoStr) {
  ...
  const cls = days === 0 ? 'due-soon' : days <= 3 ? 'due-soon' : '';
  return { text, cls };
}
